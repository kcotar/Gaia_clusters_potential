...
def eval_gaussian2D(params, xv, yv, zv, evaluate=True):
    ...
    zn = np.full_like(zv, 0.)
    # add image offset level
    zn += params['offset']
    # the following equation is the same as in the case of astropy model fitting
...
def fit_multi_gaussian2d(z_vals, x_vals, y_vals, peaks_xy, peaks_val,
    ...
    for i_p in range(n_peaks):
        ...
        fit_param.add('xs'+su, value=1., min=0.1, max=10., vary=True)
        fit_param.add('ys'+su, value=1., min=0.1, max=10., vary=True)
    ...
    fitted_curve = eval_gaussian2D(fit_res.params, x_vals, y_vals, z_vals, evaluate=False)

    return fitted_curve, fit_res
